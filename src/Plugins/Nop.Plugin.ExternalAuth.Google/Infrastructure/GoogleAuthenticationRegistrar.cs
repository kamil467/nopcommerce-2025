...
public class GoogleAuthenticationRegistrar : IExternalAuthenticationRegistrar
{
    ...
    public void Configure(AuthenticationBuilder builder)
    {
        builder.AddGoogle(GoogleDefaults.AuthenticationScheme, options =>
        {
            ...
            options.ClientId = string.IsNullOrEmpty(settings?.CliendId) ? nameof(options.ClientId) : settings.CliendId;
            options.ClientSecret = string.IsNullOrEmpty(settings?.SecretKey) ? nameof(options.ClientSecret) : settings.SecretKey;
